      ...
      this._AuthService.createUser(this.UserFormGroup.value)
        .subscribe({
          next: (data) => {
            ...
            this._AuthService.showToast("User Created Successfully", 'success', 'bottom-end');
            this.getUser();
          }, error(err) {
            console.log("Error : ", err);
              this.showToast('User Already Exists', 'danger', 'bottom-end');
          },
        })
  }
  ...
  updateUser() {
    ...
    user.Password = this.UserFormGroup.value.Password
      this._AuthService.updateUser(user)
        .subscribe({
          next: (data) => {
            this.showToast('User Updated Successfully', 'success', 'bottom-end');
            this.getUser();
          }, error(err) {
            this.showToast('User not Updated', 'success', 'bottom-end');
          },
        })
  }
